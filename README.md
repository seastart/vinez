...
**发一句工程需求，Vinez 持续执行到交付。**

Vinez 是面向团队的 AI 工程执行中枢：装在一台常开机器上的服务端（`vinezd`），把整个团队的工程需求变成持续、可治理的任务，再编排 AI agent 执行到交付。成员从聊天、Web 或终端都能发起、协作、随时接管——不必各自安装、各自配置，也不必各自维护一套 agent。
